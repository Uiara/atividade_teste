class Node:
	def __init__(self, dado, anterior = None, proximo = None, dadoMesa):
		self._dado = dado 
		self._proximo = proximo
		self._anterior = anterior
        self._dadoMesa = dadoMesa 

	def getDado(cls):
		return cls._dado
    def getDadoMesa(cls):
        return cls._dadoMesa
    
	def setDado(cls, novoDado):
		''' Este método recebe outro nó como parâmetro. '''
		cls._dado = novoDado
    def setDadoMesa(cls, novoDadoMesa):
        cls._dadoMesa = novoDadoMesa
...
class Pilha:
	...
	def removerPrimeiro(cls):
		''' Remove o primeiro elemento da lista e retorna o dado deste elemento. '''
		if not cls.isVazia():
			dadoRemovido = cls.getInicio().getDado()
